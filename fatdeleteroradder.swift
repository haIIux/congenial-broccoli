            ...
            .onTapGesture {
                ...
                case .male:
                    if chosenGoal.description.contains("Mild Weight Gain") {
                        let caloriesBeforeCalculation = Double(CaloriesMen()) // 2027
                        let mildWeightGainGoalMath = CaloriesMen() * chosenGoal.rawValue // 202.7
                        let mildWeightGainAddingOfGoal = CaloriesMen() + mildWeightGainGoalMath // 2229.7
                        let mildWeightGainActivityMath = mildWeightGainAddingOfGoal * chosenActivityLevel.rawValue // 2675.64
                        let mildWeightGainCalculation = mildWeightGainAddingOfGoal + mildWeightGainGoalMath // 2432.4
                        dailyCalories = mildWeightGainCalculation
                        print("Calories Before Calc : \(caloriesBeforeCalculation), \(chosenGoal.description) : \(chosenGoal.rawValue), Amount after addition of caloric increase : \(mildWeightGainActivityMath), \(chosenActivityLevel.description) : \(chosenActivityLevel.rawValue), Final : \(dailyCalories)")
                       
                    } else if chosenGoal.description.contains("Moderate Weight Gain") {
                        let caloriesBeforeCalculation = Double(CaloriesMen())
                        let moderateWeightGainGoalMath = CaloriesMen() * chosenGoal.rawValue
                        let moderateWeightGainAddingOfGoal = CaloriesMen() + moderateWeightGainGoalMath
                        let moderateWeightGainActivityMath = moderateWeightGainAddingOfGoal * chosenActivityLevel.rawValue
                        ...
                        print("Calories Before Calc : \(caloriesBeforeCalculation), \(chosenGoal.description) : \(chosenGoal.rawValue), Amount after addition of caloric increase: \(moderateWeightGainActivityMath), \(chosenActivityLevel.description) : \(chosenActivityLevel.rawValue),  Final : \(dailyCalories)")
                    } else {
                    let caloriesBeforeCalculation = Double(CaloriesMen())
                    let chosenGoalMath = CaloriesMen() * chosenGoal.rawValue
                    let completedGoalMathNowSubtractCaloriesForWeightLoss = CaloriesMen() - chosenGoalMath
                ...
                case .female:
                    if chosenGoal.description.contains("Mild Weight Gain") {
                        ...
                        let mildWeightGainMathMultiply = CaloriesWomen() * chosenGoal.rawValue
                        let mildWeightGainMathAddition = CaloriesWomen() + mildWeightGainMathMultiply
                        let mildDailyCalories = mildWeightGainMathAddition * chosenActivityLevel.rawValue
                        dailyCalories = mildDailyCalories
                        print("Mild Weight Gain Calories : \(dailyCalories)")
                    } else if chosenGoal.description.contains("Moderate Weight Gain") {
                        print("Moderate Weight Gain")
                        let moderateWeightGainMathMulitply = CaloriesWomen() * chosenGoal.rawValue
                        let moderateWeightGainMathAddition = CaloriesWomen() + moderateWeightGainMathMulitply
                        let moderateDailyCalories = moderateWeightGainMathAddition * chosenActivityLevel.rawValue
                        dailyCalories = moderateDailyCalories
                        print("Moderate Weight Gain Calories : \(moderateDailyCalories)") // 2229
                    } else {
                    let chosenGoalMath = CaloriesWomen() * chosenGoal.rawValue
                    let completedGoalMathNowSubtractCaloriesForWeightLoss = CaloriesWomen() - chosenGoalMath
                    dailyCalories = completedGoalMathNowSubtractCaloriesForWeightLoss * chosenActivityLevel.rawValue
...
struct MacroCalculatorView_Previews: PreviewProvider {
    static var previews: some View {
        MacroCalculatorView(chosenActivityLevel: .sedentary, genderSelector: GenderPickerSelector.male, goalSelector: .moderateWeightGain)
    }
}
